...
const ServiceCard: FunctionComponent<ServiceCardProps> = (props) => {
  const {service, likeToggle} = props;
  ...
  const showLikeBtn = token && service.likes?.includes(decode?._id as string);
  const isLiked = showLikeBtn ? 'ri-heart-3-fill' : 'ri-heart-3-line';
        ...
        <div className='relative flex flex-row items-center gap-4'>
          ...
          <div className='flex flex-col gap-0.5 text-sm text-slate-700 dark:text-slate-400'>
            ...
            <div className='mt-1.5'>
              ...
              >
                <h3 className='capitalize mb-1 font-semibold'>
                  {service.name}
                </h3>
          ...
          <div className='absolute right-0 bottom-0'>
            <Button
              size='sm'
              onClick={() => navigate(`/book-appointment/${service._id}`)}
            >
              Book Now
            </Button>
            ...
            {token && (
              <i
                onClick={() => likeToggle(service._id as string)}
                className={`${isLiked} ml-4 text-rose-600 cursor-pointer`}
              />
            )}
          </div>
        </div>
